...
class PosRepository(private val posDao: PosDao) {
    ...
    // Shifts
    val allShifts: Flow<List<ShiftEntity>> = posDao.getAllShifts()
    suspend fun getActiveShiftForKasir(username: String): ShiftEntity? = posDao.getActiveShiftForKasir(username)
    suspend fun getAnyActiveShift(): ShiftEntity? = posDao.getAnyActiveShift()
    suspend fun getShiftById(id: Long): ShiftEntity? = posDao.getShiftById(id)
    suspend fun insertShift(shift: ShiftEntity): Long = posDao.insertShift(shift)
    suspend fun updateShift(shift: ShiftEntity) = posDao.updateShift(shift)
    suspend fun deleteShift(id: Long) = posDao.deleteShift(id)
    ...
    suspend fun getAllShiftsDirect(): List<ShiftEntity> = posDao.getAllShiftsDirect()
    ...
    suspend fun insertShifts(shifts: List<ShiftEntity>) = posDao.insertShifts(shifts)
