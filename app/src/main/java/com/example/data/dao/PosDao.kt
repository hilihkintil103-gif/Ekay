...
interface PosDao {
    ...
    @Query("SELECT * FROM shifts WHERE status = 'OPEN' AND kasirUsername = :username LIMIT 1")
    suspend fun getActiveShiftForKasir(username: String): ShiftEntity?

    @Query("SELECT * FROM shifts WHERE status = 'OPEN' LIMIT 1")
    suspend fun getAnyActiveShift(): ShiftEntity?
    ...
    suspend fun getAllShiftsDirect(): List<ShiftEntity>
